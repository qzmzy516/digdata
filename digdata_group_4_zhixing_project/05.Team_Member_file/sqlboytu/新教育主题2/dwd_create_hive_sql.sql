-- dwd层建表语句
DROP TABLE if exists     dwd.dim_scrm_department;
CREATE TABLE dwd.dim_scrm_department
(
    id               INT                  COMMENT '部门id',        
...
-- 申诉表
DROP TABLE if exists     dwd.fact_customer_appeal;
CREATE TABLE dwd.fact_customer_appeal
(
    id                             INT,
...
partitioned by (dt string) row format delimited fields terminated by '\t' stored as orc tblproperties ('orc.compress'='snappy');

-- 校区表
DROP TABLE if exists  dwd.dim_itcast_school;
CREATE TABLE dwd.dim_itcast_school
(
    id               int ,
    create_date_time string                COMMENT '创建时间',
...
-- 学科表
DROP TABLE if exists  dwd.dim_itcast_subject;
CREATE TABLE dwd.dim_itcast_subject
(
    id                  INT ,            
    create_date_time    string             COMMENT '创建时间',
    update_date_time    string             COMMENT '最后更新时间',
...
-- 学员表
DROP TABLE if exists  dwd.dim_customer;
CREATE TABLE dwd.dim_customer
(
    id                       INT ,
...
-- 员工表
DROP TABLE if exists     dwd.dim_employee;
...
-- 线索表
 DROP TABLE if exists dwd.dim_customer_clue;
CREATE TABLE dwd.dim_customer_clue
(
...
partitioned by (dt string) row format delimited fields terminated by '\t' stored as orc tblproperties ('orc.compress'='snppy');



DROP TABLE if exists  dwd.fact_customer_relationship;
CREATE TABLE dwd.fact_customer_relationship
(
